    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields([
          ...
          { name: 'Expiration Date', value: coupon.expiration_date ? new Date(coupon.expiration_date).toLocaleString() : 'No Expiration Date' },
          { name: 'Uses', value: `${coupon.uses}/${coupon.max_uses || '∞'}` },
          { name: 'Allowed Emails', value: coupon.allowed_emails.length ? coupon.allowed_emails.join(', ') : 'All Emails' },
          { name: 'Products', value: coupon.products.length ? coupon.products.map(product => product.name).join(', ') : 'All Products' }
        ]);
